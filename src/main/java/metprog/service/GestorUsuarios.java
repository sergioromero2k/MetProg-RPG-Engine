...
public class GestorUsuarios {
   ...
   * @return true si la baja se realizó correctamente.
   */
   ...
   * @return el usuario autenticado, o {@code null} si las credenciales son incorrectas.
   */
   ...
   * @return true si el usuario existe y se realizó la asignación.
   */
   ...
   *
   * @param nick nick del usuario.
   * @return true si el usuario existe y tenía personaje.
   */
   ...
   *
   * @return lista ordenada de usuarios que tienen personaje.
   */
  public List<Usuario> getRankingGlobal() {
    ...
    conPersonaje.sort(Comparator.comparingInt(this::calcularOroAcumulado).reversed());
    return conPersonaje;
  }

  private int calcularOroAcumulado(Usuario u) {
    int total = 0;
    for (Usuario.RegistroOro r : u.getHistorialOro()) {
      if (r.getCantidad() > 0) {
        total += r.getCantidad();
      }
    }
  ...
  /**
   * Reemplaza la lista interna de usuarios.
   *
  ...
  /**
   * Reemplaza la lista interna de operadores.
   *
